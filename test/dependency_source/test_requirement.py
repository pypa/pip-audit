...
def _check_fixes(
    ...
    for input_req, req_path in zip(input_reqs, req_paths, strict=False):
    ...
    for expected_req, req_path in zip(expected_reqs, req_paths, strict=False):
...
def test_requirement_source_fix_parse_failure(monkeypatch, req_file):
    ...
    for expected_req, req_path in zip(input_reqs, req_paths, strict=False):
        with open(req_path) as f:
            assert expected_req == f.read().strip()
